let apiUtils = {
    // /store/fz/1/5   根据城市id获取选址点
    getStoreListInfoByCityId(countyId,page,size=5) {
        if(vue.requestState) {
            vue.requestState.storeList = 'start'
        }
        console.log('正在获取商铺列表(无县名)',`/store/${countyId}/${page}/${size}`)
        return new Promise( (resolve,reject) => {
            fetch(BASE_URL+`/store/${countyId}/${page}/${size}`,{
                mode: 'cors'
            })
            .then(res => res.json())
            .then(res => {
                // 将获取的数据转化为后端工程师第一次定义的数据结构 (越改越离谱)
                let list = []
                res.data.forEach( storeData => {
                    list.push({
                        store: {
                            ...
                            countyId: storeData.countyId,
                            location: storeData.location,
                            storeTitle: storeData.storeTitle,
                            storeImg: storeData.storeImg,
                            storeArea: storeData.storeArea,
                            storeAddress: storeData.storeAddress,
                            storeRent: storeData.storeRent,
                            publisherName: storeData.publisherName,
                            publisherTel: storeData.publisherTel,
                            rentState: storeData.rentState,
                ...
                vue.requestState.storeList = 'end'
                console.log('获取商铺列表(无县名) 完成',`/store/${countyId}/${page}/${size}`,list)
                resolve(list)
            })
            .catch(err => reject('err:', err))
        })
    },
    // /store/fz/lj/1/5   根据城市id和地区id获取选址点
    getStoreListInfoByFullId(cityId,countyId,page,size=5) {
        if(vue.requestState) {
            vue.requestState.storeList = 'start'
        }
        console.log(`正在获取商铺列表(县id: ${countyId})`,`/store/${cityId}/${countyId}/${page}/${size}`)
        return new Promise( (resolve,reject) => {
            fetch(BASE_URL+`/store/${cityId}/${countyId}/${page}/${size}`,{
                mode: 'cors'
            })
            .then(res => res.json())
            .then(res => {
                ...
                vue.requestState.storeList = 'end'
                console.log(`正在获取商铺列表(县id: ${countyId}) 完成`,`/store/${cityId}/${countyId}/${page}/${size}`,list)
                resolve(list)
            })
            .catch(err => reject('err:', err))
        })
    },
    // /county/fz    根据城市id获取地区列表
    getCountyListInfo(cityId) {
        console.log(`正在获取城市的所有县(城市id: ${cityId})`,`/county/${cityId}`)
        return new Promise( (resolve,reject) => {
            fetch(BASE_URL+`/county/${cityId}`,{
                mode: 'cors'
            })
            .then(res => res.json())
    ...
    /*
        返回结果为
        [
            {"count": 61, "lat": 30.5984667364009, "lng":114.311581554732},
            {"count": 61, "lat": 30.5984667364009, "lng":114.311581554732}
        ]
    */
    // /store/fz/lianjiang/powers
    getHotMapArr(cityId,countyId) {
        return new Promise( (resolve,reject) => {
            return new Promise( (resolve,reject) => {
                fetch(BASE_URL+`/${cityId}/${countyId}/powers`,{
                    mode: 'cors'
                })
                .then(res => res.json())
                .then(res => resolve(res.data))
...
let apiUtils2 = {
    getStoreListInfoByCityId(countyId,page,size=5) {
        ...
        return new Promise( (resolve,reject) => {
            if(page === 1) {
                console.log('获取商铺列表(无县名) 完成',`/store/${countyId}/${page}/${size}`,mockData.storeList1)
                resolve(mockData.storeList1)
            }
            else if(page === 2) {
                console.log('获取商铺列表(无县名) 完成',`/store/${countyId}/${page}/${size}`,mockData.storeList2)
                resolve(mockData.storeList2)
            }
        })
    },
    getStoreListInfoByFullId(cityId,countyId,page,size=5) {
        ...
        return new Promise( (resolve,reject) => {
            if(page === 1) {
                console.log(`正在获取商铺列表(县id: ${countyId}) 完成`,`/store/${cityId}/${countyId}/${page}/${size}`,mockData.storeList1)
                resolve(mockData.storeList1)
            }
            else if(page === 2) {
                console.log(`正在获取商铺列表(县id: ${countyId}) 完成`,`/store/${cityId}/${countyId}/${page}/${size}`,mockData.storeList2)
                resolve(mockData.storeList2)
            }
        })
    },
    getCountyListInfo(cityId) {
        console.log('正在获取countyList: ')
        return new Promise( (resolve,reject) => {
            console.log('获取countyList 完成: ',mockData.countyList)
            resolve(mockData.countyList)
        })
    },
    getHotMapArr(cityId,countyId) {
        return new Promise( (resolve,reject) => {
...
// url = 'xxx?mode=mock'  使用本地的假数据
{
    ...
    if(getQueryString('mode') === 'mock') {
        apiUtils = apiUtils2
    }
}

function test() {
    // apiUtils.getStoreListInfoByCityId('lj',1)
    // .then(data => console.log(data))

    // 这里有点bug, 去掉下面两行就报错
    apiUtils.getCountyListInfo('fz')
    .then(data => console.log(data))
}
test()
...
//     /* 
//         localStorage:
//             addressName: province+'|'+city+'|'+county,
...
//     },
//     strParse2Pos(str) {
//         let [longitude,latitude] = str.split('|')
//         return {
...
const MapUtils = {
    ...
    // /*
    //     参数: str="119.335730,26.079770", score=5.95
    //     返回: {
    //         lng: 119.335730,
    //         lat: 26.079770
    //     }
    // */
    // posStr2HotMapItemObj(str,score) {
